...
abstract class HashSink implements Sink<List<int>> {
  ...
  HashSink(this._sink, int chunkSizeInWords, {Endian endian = Endian.big, int signatureBytes = 8})
    : _endian = endian,
...
class _MD5Sink extends HashSink {
  ...
  void updateHash(Uint32List chunk) {
    ...
    void round(int i) {
      ...
      b = add32(b, rotl32(add32(add32(a, e), add32(_noise[i], chunk[f])), _shiftAmounts[i]));
...
class _MD5 extends Hash {
  ...
  @override
  ByteConversionSink startChunkedConversion(Sink<Digest> sink) => ByteConversionSink.from(_MD5Sink(sink));
}
